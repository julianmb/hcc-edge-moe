...
\renewcommand{\maketitle}{
    ...
    \begin{center}
        \vspace*{1em}
        {\LARGE \bfseries \color{PreprintBlue} Heterogeneous Compute Cascades: \\[0.3em] A Cost-Effective Architectural Solution for \\[0.3em] 320B-Parameter MoE Inference on Edge Clusters \par}
        \vspace{1.5em}
...
\textbf{Abstract.} The democratization of frontier artificial intelligence is fundamentally blocked by the capital expenditure (CAPEX) and thermodynamic footprint required to host models exceeding 100 billion parameters. While a dual-node 16-Core Ryzen APU cluster provides a 256 GB unified memory pool---structurally capable of holding a 320B-parameter GLM-5.3-Flash MoE (18B active per token, $\sim$158 GB checkpoint under ROCmFP4)---traditional distributed execution frameworks over consumer USB4 bridges suffer from catastrophic network pipeline bubbles driven by TCP/IP OS kernel latency.

We introduce the \textit{Heterogeneous Compute Cascade} (HCC), a software architecture designed to convert this hardware limitation into a strategic advantage. Rather than treating heterogeneous IP blocks (CPU, iGPU, NPU) as a uniform compute pool, HCC enforces strict silicon-to-task affinity. The proposed architecture leverages GLM-5.3-Flash's hybrid attention (34 KDA linear attention layers with $O(1)$ constant state + 11 DSA sparse MLA layers) and native Multi-Token Prediction (MTP) with an analytical MoE verification scaling factor $v_\gamma$. The NPU offloads contextual preprocessing and index scoring, while the iGPU executes high-throughput verification. These accelerator paths are design targets: the current runtime validates a stop-and-wait typed protocol in simulation, while physical NPU compression and overlap remain unimplemented.

Furthermore, to ensure the KV cache does not exhaust unified memory during long-context execution, the architecture combines GLM-5.3-Flash's 11-layer DSA footprint with 3-bit TurboQuant KV compression via Walsh-Hadamard rotations. The target software stack specifies kernel-level DMA-BUF descriptors to bridge the iGPU (via MIGraphX) and NPU (via Vitis AI); the present prototype uses framed TCP plus a host-memory \texttt{memfd}/\texttt{mmap} scaffold and does not yet implement cross-driver zero-copy. Our analytical model projects a baseline decode throughput of 21.9 tok/s (9.7 GB active weight read at 212 GB/s) scaling to 30.1 tok/s under speculative verification ($v_2 \approx 1.49$, $\gamma=2$), with KV cache consumption of only $\sim$1.27 GB per session at 200K context (or 5.18 GB under full MLA). Theoretical analysis indicates a hardware cost of \$20.31 per GB of addressable memory---representing a projected 92\% cost reduction and 96\% power reduction versus a turnkey DGX A100 deployment. This work demonstrates, through formal mathematical analysis, that intelligent orchestration rather than raw interconnect bandwidth is the key architectural principle for enabling massive-scale AI inference at the edge.
\end{tcolorbox}
\end{center}

\vspace{0.5cm}
\noindent\textbf{Keywords:} distributed inference, latency hiding, AMD 16-Core Ryzen APU, XDNA 2, TurboQuant, GLM-5.3-Flash, Mixture of Experts (MoE), TCO optimization, edge computing, speculative decoding
...
The relentless scaling laws governing Large Language Models (LLMs) have precipitated a crisis in hardware accessibility. As the frontier of intelligence pushes past 100 billion parameters (e.g., Llama-3-400B, Grok-1.5, GLM-5.3-Flash), the memory required to simply load model weights often exceeds 200 GB. Single consumer graphics processing units (GPUs) currently peak at 24 GB to 48 GB of Video RAM (VRAM). To execute these models locally, researchers and enterprises are forced into traditional data center scaling paradigms---utilizing specialized server-grade interconnects such as NVIDIA's NVLink across multi-GPU arrays (e.g., an $8\times$ H100 SXM node).
...
\subsection{Mixture of Experts and GLM-5.3-Flash Architecture}
...
\end{equation}
where $E$ is the total number of experts, $E_i(x)$ is the expert output, and $G(x)_i$ is the gating probability. To enforce sparsity, a Top-$K$ routing mechanism is employed:
\begin{equation}
    G(x) = \text{Softmax}\left(\text{Top}_K(x \cdot W_g + \epsilon \cdot \text{Softplus}(x \cdot W_{\text{noise}}))\right)
\end{equation}

In this work, we analyze the frontier open-weight \textbf{GLM-5.3-Flash} model \cite{glm5} ($\sim$320B total parameters, $E=288$ routed experts, $K=8$ active experts per token, $\sim$18B active parameters per forward pass, $L=45$ layers, hidden dimension $H=4096$). GLM-5.3-Flash introduces a hybrid attention and recurrent architecture comprising \textbf{34 Kangaroo Linear Attention (KDA)} layers that maintain fixed $O(1)$ recurrent hidden state, paired with \textbf{11 DeepSeek Sparse Attention (DSA)} layers implementing Multi-head Latent Attention (MLA) with Manifold-Constrained Hyper-Connections (mHC). 

GLM-5.3-Flash's MLA compresses attention state into low-rank KV latent vectors ($\text{kv\_lora\_rank}=512$, $\text{qk\_rope\_head\_dim}=64$, $d_{kv}=576$), storing only 576 values per token per DSA layer. Because linear attention layers maintain $O(1)$ state across the entire context window, sequence-length-dependent KV cache memory is stored \textit{only} for the 11 DSA layers, compressing the long-context memory footprint by an additional $\sim$4.1$\times$ beyond standard MLA. Furthermore, GLM-5.3-Flash incorporates a native Multi-Token Prediction (MTP) head, enabling verified parallel speculative generation without requiring an external draft model.

We target the \textbf{Q4\_0\_ROCMFP4\_STRIX\_LEAN} quantization format ($\sim$4.3 bpw, 158.0 GB checkpoint gross, $\sim$79.0 GB per node). In a dual-node 256 GB cluster, this leaves $\sim$98 GB of gross memory headroom across both nodes for KV cache and OS overhead, natively hosting the full 320B model without requiring destructive expert pruning. For single-node 128 GB deployments, aggressive quantizations such as \texttt{UD-IQ3\_XXS} ($\sim$120 GB) or \texttt{STRIX-BALANCED} ($\sim$72 GB) provide compact alternatives.
...
\textbf{MoE compression and hybrid attention.} DeepSeek-V3 \cite{deepseek} employs expert parallelism with auxiliary-loss-free load balancing. GLM-5.3-Flash \cite{glm5} replaces pure transformer attention with a hybrid 34-layer KDA linear attention and 11-layer DSA/MLA topology, drastically shrinking KV memory requirements.

\textbf{KV cache quantization.} QuaRot \cite{quarot} applies Walsh-Hadamard rotations to remove outliers before 4-bit quantization. TurboQuant \cite{turboquant} extends this to near-optimal 3-bit vector quantization with QJL residual correction \cite{qjl}. KIVI \cite{kivi} achieves 2-bit KV quantization asymmetrically. HCC integrates TurboQuant-class compression with GLM-5.3-Flash's 11 DSA layers.

\textbf{Application-layer context reduction.} While HCC addresses the physical interconnect bottleneck at the hardware layer, application-layer frameworks such as Cascaded Signal Refinement (CSR) \cite{csr} address the \textit{logical} context bottleneck. CSR employs a three-layer pipeline---deterministic pre-filtering, small-model LLM screening, and deep analysis---to reduce enterprise compliance corpora by 96.9\% prior to expensive inference. HCC provides the natural zero-data-transit hardware substrate for such sovereign, multi-layered inference pipelines.
...
For GLM-5.3-Flash with hidden dimension $H = 4096$ utilizing FP16 precision:
\begin{itemize}
    \item \textbf{Decode Phase Bubble:} Transmitting a single token's activation requires $1 \times 4096 \times 2 = 8$ KB. Traversing an unoptimized OS TCP/IP stack establishes a latency floor of $\sim$0.5 ms per node crossing \cite{geerling2025} (reduced to $\sim$17 $\mu$s with the kernel tuning described in Section 5.1). Node 2 is idle (a pipeline bubble) while waiting for this transmission.

    \item \textbf{Prefill Phase Bubble:} Transmitting a 100,000-token context requires $100,000 \times 4096 \times 2 \approx 0.819$ GB of data. Utilizing IP-over-Thunderbolt (MTU 9000), effective TCP throughput peaks at approximately 1.25 GB/s.
    \begin{equation}
        T_{\text{stall}} = \frac{0.819 \text{ GB}}{1.25 \text{ GB/s}} + \text{TCP Overhead} \approx 0.655 \text{ seconds}
    \end{equation}
    This dictates a $\sim$0.66 second pure network stall \textit{per RPC boundary}.
\end{itemize}
...
\fill[PreprintBlue!40] (0.5, 2.3) rectangle (3, 3) node[midway, font=\scriptsize] {Compute L0--22};
\fill[gray!20] (3, 2.3) rectangle (4.5, 3) node[midway, font=\scriptsize] {\textit{Wait}};
\fill[PreprintBlue!40] (4.5, 2.3) rectangle (7, 3) node[midway, font=\scriptsize] {Compute L0--22};
\fill[gray!20] (7, 2.3) rectangle (8.5, 3) node[midway, font=\scriptsize] {\textit{Wait}};
\fill[PreprintBlue!40] (8.5, 2.3) rectangle (11, 3) node[midway, font=\scriptsize] {Compute L0--22};
...
\end{tikzpicture}
\caption{Pipeline bubble in na\"ive RPC execution. Node~2 idles during Node~1 computation and USB4 transfer, wasting $\sim$50\% of cluster compute. HCC is designed to eliminate this via speculative pre-computation and pipelining.}
...
\noindent Crucially, the HCC architecture addresses memory contention through \textbf{physical isolation}. By partitioning the target MoE pipeline across two nodes and utilizing speculative generation (via GLM-5.3-Flash's native MTP head or asynchronous NPU pre-filtering), the workload utilizes \textit{two independent} 256-bit memory buses. The 17 $\mu$s USB4 link acts as the synchronization bridge, allowing execution to proceed without starving memory bandwidth.

A naive alternative would be to execute uncoordinated auxiliary drafting on Node~1's iGPU. However, in our pipeline-parallel topology (Figure 2), Node~1's iGPU is fully engaged executing Layers 0--22 of the target model. Tasking it with auxiliary draft generation would introduce context-switching and memory contention against the main model. The XDNA~2 NPU is available on Node~1 for asynchronous preprocessing without disrupting the main iGPU memory pipeline.
...
\node[box, below left=0.8cm and 0.5cm of input, fill=HighlightGreen!15, draw=HighlightGreen, text width=4.5cm] (npu) {\textbf{XDNA 2 NPU (Node 1)}\\[2pt] Context Preprocessor / Draft};
...
\node[box, below right=0.8cm and 0.5cm of input, fill=AlertRed!15, draw=AlertRed, text width=4cm] (gpu1) {\textbf{iGPU Node 1}\\[2pt] MoE Layers 0--22\\DSA / MLA KV Cache};
\node[box, below=0.6cm of gpu1, fill=AlertRed!15, draw=AlertRed, text width=4cm] (gpu2) {\textbf{iGPU Node 2}\\[2pt] MoE Layers 23--44\\DSA / MLA KV Cache};
...
% Standard RPC: T = tokens * H * 2 / BW + base = x * 4096 * 2 / 1.25e9 + 0.1 = x * 0.00000655 + 0.1
\addplot[thick, AlertRed, domain=0:128000] {x * 0.00000655 + 0.1};
% HCC: T_NPU(x) + T_net(alpha*x) + overhead = x*0.0000005 + 0.2*x*0.00000655 + 0.15 = x * 0.00000181 + 0.15
\addplot[thick, HighlightGreen, domain=0:128000] {x * 0.00000181 + 0.15};
...
\section{Mathematical Formulation of Speculative Speedup and MoE Scaling}

During token generation, HCC evaluates speculative verification to amortize USB4 network traversal. Rather than relying solely on separate-device sequential execution, generation can follow speculative decoding \cite{spec_decode} or native Multi-Token Prediction (MTP).

The topological paradigm of partitioning speculative decoding across heterogeneous compute units has been explored in frameworks such as Mirror Speculative Decoding \cite{mirror_spec}. However, on unified memory architectures (e.g., AMD APUs), executing draft models on the NPU while verifying on the iGPU shares the same 212 GB/s LPDDR5x bus. We treat critical-path NPU drafting as a gated falsification test: if bus contention degrades iGPU throughput, the NPU is redirected to off-critical-path tasks (such as DSA sparse-attention index scoring or multimodal preprocessing), while GLM-5.3-Flash's native on-chip MTP head performs speculative drafting.

\begin{algorithm}[ht!]
\caption{HCC Speculative Network Amortization}\label{alg:spec_decode}
\begin{algorithmic}[1]
\Require Main MoE model $\mathcal{M}$ (GLM-5.3-Flash), Draft generator $\mathcal{S}$ (MTP head or NPU).
\While{generation incomplete}
    \State Generate $\gamma$ draft tokens $\hat{x}_{t}, \dots, \hat{x}_{t+\gamma-1}$ using $\mathcal{S}$.
    \State Pass sequence through $\mathcal{M}$ on iGPU cluster as a single parallel verification batch.
    \State Incurs $1 \times$ TCP/IP RPC latency for the entire draft batch.
...
The expected number of accepted tokens $\mathbb{E}[k]$ per verification step with acceptance rate $\alpha$ is:
\begin{equation}
    \mathbb{E}[k] = \frac{1 - \alpha^{\gamma+1}}{1 - \alpha}
\end{equation}

In standard dense LLMs, verifying $\gamma$ draft tokens simultaneously in parallel requires reading the weight matrices only once ($v_\gamma \approx 1.0$), since all tokens share the identical parameter tensors in compute-bound matrix multiplications. 

However, in \textbf{Mixture-of-Experts (MoE)} architectures, each draft token routes to a distinct subset of $K$ experts. When verifying $\gamma$ draft tokens simultaneously, the target model must read the \textit{union} of active experts across all $\gamma$ positions from main memory. Assuming router selections have partial correlation across successive positions, the expected number of unique active experts expands sublinearly with $\gamma$:
\begin{equation}
    v_\gamma = \frac{\mathbb{E}\left[\left|\bigcup_{i=1}^{\gamma}\mathcal{E}_i\right|\right]}{K}
\end{equation}
For GLM-5.3-Flash ($E=288$ routed experts, $K=8$ active per token), analytical and empirical active-expert scaling yields:
\begin{equation}
    v_1 = 1.00, \quad v_2 \approx 1.49, \quad v_3 \approx 1.98, \quad v_4 \approx 2.44, \quad v_5 \approx 2.87
\end{equation}

Incorporating the MoE verification cost factor $v_\gamma$ and relative drafting cost ratio $c/C$, the true theoretical speedup $S$ of the speculative MoE pipeline is:
\begin{equation}
    S = \frac{\mathbb{E}[k]}{v_\gamma + \gamma \cdot \frac{c}{C}} = \frac{1 - \alpha^{\gamma+1}}{(1 - \alpha)\left(v_\gamma + \gamma \frac{c}{C}\right)}
\end{equation}

Equation (7) exposes a fundamental microarchitectural constraint for MoE inference: large draft lengths (e.g., $\gamma=5$) that succeed on dense models are counterproductive on MoE. With $\alpha=0.70$ and $c/C=0.05$, a naive choice of $\gamma=5$ with $v_5=2.87$ yields:
\begin{equation}
    S_{\gamma=5} = \frac{2.941}{2.87 + 0.25} = \frac{2.941}{3.12} \approx 0.942\times < 1.0\times
\end{equation}
resulting in a \textbf{performance regression} below autoregressive decoding. 

Conversely, bounding the speculative draft length to $\gamma=2$ (matching GLM-5.3-Flash's native MTP head) limits expert inflation to $v_2 \approx 1.49$, producing robust positive speedup:
\begin{equation}
    S_{\gamma=2} = \frac{2.190}{1.49 + 2 \times 0.05} = \frac{2.190}{1.59} \approx \textbf{1.377}\times
\end{equation}
Amortizing the 17 $\mu$s USB4 link across $\mathbb{E}[k] = 2.19$ accepted tokens effectively hides network serialization overhead.

\begin{figure}[ht!]
\centering
\begin{tikzpicture}
\begin{axis}[
    title={Speculative Decoding Speedup vs Acceptance Rate in MoE},
...
% MoE gamma = 2, v_2 = 1.49, c/C = 0.05
\addplot[thick, PreprintBlue, domain=0:0.99, samples=100] {(1 - x^3) / ((1 - x) * 1.59)};
\addlegendentry{MoE Bounded ($\gamma = 2, v_2 = 1.49, c/C = 0.05$)}

% Dense reference gamma = 5, v_5 = 1.0, c/C = 0.05
\addplot[thick, dashed, HighlightGreen, domain=0:0.99, samples=100] {(1 - x^6) / ((1 - x) * 1.25)};
\addlegendentry{Dense Reference ($\gamma = 5, v=1.0, c/C = 0.05$)}

% MoE unconstrained gamma = 5, v_5 = 2.87, c/C = 0.05
\addplot[thick, dashdotted, AlertRed, domain=0:0.99, samples=100] {(1 - x^6) / ((1 - x) * 3.12)};
\addlegendentry{MoE Naive ($\gamma = 5, v_5 = 2.87, c/C = 0.05$)}

\draw[AlertRed, thick, dotted] (axis cs:0, 1) -- (axis cs:1, 1) node[pos=0.85, below, font=\small] {Break-even ($S=1$)};
\end{axis}
\end{tikzpicture}
\caption{Theoretical speedup of speculative decoding with MoE active-expert scaling factor $v_\gamma$. While unconstrained $\gamma=5$ drops below break-even on MoE due to expert union expansion ($v_5=2.87$), bounded $\gamma=2$ maintains consistent positive speedup ($S \approx 1.38\times$ at $\alpha=0.7$).}
\label{fig:spec_speedup}
\end{figure}

\section{Long Context Scaling: Hybrid KDA+DSA and TurboQuant}

GLM-5.3-Flash introduces a hybrid attention architecture that fundamentally redefines long-context memory requirements. Of its 45 transformer layers, \textbf{34 Kangaroo Linear Attention (KDA)} layers maintain a fixed $O(1)$ recurrent hidden state across the entire context window, requiring zero sequence-length-dependent KV memory growth. Sequence-dependent KV cache is retained \textit{only} for the \textbf{11 DeepSeek Sparse Attention (DSA)} layers, which implement Multi-head Latent Attention (MLA) with $\text{kv\_lora\_rank} + \text{qk\_rope\_head\_dim} = 512 + 64 = 576$ values per token per layer (versus $2 \times H = 8{,}192$ in standard MHA).

In a balanced 2-node cluster, each node hosts $11 / 2 = 5.5$ DSA layers. At 200K tokens, the per-node FP16 KV cache for GLM-5.3-Flash is:
\begin{equation}
    \text{KV}_{\text{KDA+DSA}} = 200{,}000 \times 5.5 \times 576 \times 2 \text{ bytes} \approx 1.27 \text{ GB}
\end{equation}
Even under full MLA across all 45 layers (22.5 per node), the footprint is only:
\begin{equation}
    \text{KV}_{\text{MLA, 45}} = 200{,}000 \times 22.5 \times 576 \times 2 \text{ bytes} \approx 5.18 \text{ GB}
\end{equation}
By comparison, traditional Multi-Head Attention (MHA) would demand $200{,}000 \times 22.5 \times 8192 \times 2 \approx 73.7$ GB per node, immediately exhausting available memory. GLM-5.3-Flash's hybrid attention achieves a $\sim$58$\times$ reduction over standard MHA \textit{without any quantization}.

For deployments targeting \textit{ultra-long contexts up to 1M tokens} or maximizing concurrent sessions, TurboQuant \cite{turboquant} provides further compression. Standard round-to-nearest quantization fails at $\leq$4 bits because LLM activations exhibit severe \textit{outlier channels}.
...
\end{equation}
This yields an \textbf{unbiased} inner product estimator at total bit-width $b$, storing only sign bits with zero quantization overhead. At 3.5 bits, TurboQuant achieves \textbf{identical} LongBench-E scores to full FP16 (50.06 vs 50.06) and a Needle-in-a-Haystack retrieval score of 0.997 at 104K tokens \cite{turboquant}. Applied to GLM-5.3-Flash's 5.5 DSA layers per node, 3-bit TurboQuant shrinks the 200K KV cache to just:
\begin{equation}
    \text{KV}_{\text{TurboQuant}} = 200{,}000 \times 5.5 \times 576 \times 0.375 \text{ bytes} \approx 0.238 \text{ GB} \quad (238 \text{ MB})
\end{equation}
...
\begin{tikzpicture}
\begin{axis}[
    title={Per-Node KV Cache (GLM-5.3-Flash, $d_{kv}\!=\!576$)},
    xlabel={Context Length (Tokens)},
...
% Full MLA 45 layers (22.5/node) FP16: tokens * 22.5 * 576 * 2 / 1e9
\addplot[thick, gray, domain=0:200000] {x * 22.5 * 576 * 2 / 1000000000};
\addlegendentry{Full MLA 45-Layer FP16 (22.5 L/node)}

% Hybrid KDA+DSA (5.5 DSA L/node) FP16: tokens * 5.5 * 576 * 2 / 1e9
\addplot[thick, PreprintBlue, domain=0:200000] {x * 5.5 * 576 * 2 / 1000000000};
\addlegendentry{GLM-5.3-Flash Hybrid FP16 (5.5 DSA L/node)}

% Hybrid KDA+DSA TurboQuant 3-bit: tokens * 5.5 * 576 * 0.375 / 1e9
\addplot[thick, dashed, HighlightGreen, domain=0:200000] {x * 5.5 * 576 * 0.375 / 1000000000};
\addlegendentry{Hybrid + TurboQuant 3-bit}

\end{axis}
\end{tikzpicture}
\caption{Idealized per-node KV cache scaling for GLM-5.3-Flash. Because 34 KDA layers retain $O(1)$ constant state, sequence KV cache scales only across the 11 DSA layers (5.5 per node, blue), consuming just 1.27 GB at 200K tokens. With 3-bit TurboQuant (green dashed), the footprint drops to 238 MB per node.}
\label{fig:kvcache}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Execution Providers (planned):} The intended deployment compiles GLM-5.3-Flash shards for the iGPU via \texttt{MIGraphX} and optionally utilizes XDNA~2 via \texttt{Vitis AI (VOE)} for context pre-filtering and DSA indexer scoring. The repository currently validates local llama.cpp execution and a simulated distributed protocol; it does not contain pre-compiled GLM-5.3-Flash shards.
...
\begin{table}[ht!]
\centering
\caption{Turnkey system comparison for hosting 300B+ MoE models (e.g., GLM-5.3-Flash, Q1 2026 pricing).\protect\footnotemark}
\vspace{0.2cm}
...
\subsection{Cost per Token: The Speculative Dividend}

With GLM-5.3-Flash's 18B active parameters quantized in ROCmFP4 ($\approx 0.539$ bytes/weight), each forward token decode reads approximately $18.0 \times 0.539 \approx 9.70$ GB of active weights. Over the 212 GB/s sustained memory bus, the single-node baseline decode throughput roofline is:
\begin{equation}
    T_{\text{base}} = \frac{212 \text{ GB/s}}{9.70 \text{ GB}} \approx 21.9 \text{ T/s}
\end{equation}
Under HCC speculative verification ($\gamma = 2$, $\alpha = 0.7$, $v_2 \approx 1.49$, $c/C = 0.05$), the projected effective decode throughput expands by $S \approx 1.377\times$:
\begin{equation}
    T_{\text{HCC}} = 21.9 \times \frac{1 - 0.7^{3}}{(1-0.7)(1.49 + 2 \times 0.05)} = 21.9 \times 1.377 \approx \textbf{30.1 T/s}
\end{equation}

\noindent This projected throughput depends on: (1) target execution at the configured 212 GB/s memory-bandwidth input, (2) verified speculative generation with bounded $\gamma=2$ and $v_2 \approx 1.49$, and (3) a dual-USB4 link modeled at 45 Gbps / 17 $\mu$s RTT. Physical validation of all components together on GLM-5.3-Flash remains part of the experimental roadmap.
...
\midrule
1$\times$ HCC + Speculative & 30.1$^\dagger$ & 2.60 & \$6,147 & \textbf{\$2.16} \\
1$\times$ HCC (Baseline) & 21.9$^\dagger$ & 1.89 & \$6,147 & \$2.97 \\
DGX A100 (used, batched) & 500$^*$ & 43.2 & \$125,623 & \$2.66 \\
DGX H100 (new, batched) & 1,000$^*$ & 86.4 & \$290,206 & \$3.07 \\
\textbf{20$\times$ HCC fleet} & \textbf{602}$^{\dagger\dagger}$ & \textbf{52.0} & \textbf{\$122,940} & \textbf{\$2.16} \\
\bottomrule
...
\textbf{(1) HCC beats both DGX configurations on cost-per-token.} A single HCC unit at \$2.16/M undercuts the DGX A100 at \$2.66/M and the DGX H100 at \$3.07/M. The H100's 3.5--4.5$\times$ GPU price premium buys only $\sim$2$\times$ throughput, making it the \textit{least} cost-efficient option per token.

\textbf{(2) HCC scales linearly with zero infrastructure cost.} A fleet of 20 HCC units exceeds the DGX A100's aggregate throughput (602 vs.\ 500 T/s) at comparable total cost (\$122,940 vs.\ \$125,623), while consuming 26\% less power (4,800W vs.\ 6,500W) and requiring no data center, HVAC, or 3-phase electrical infrastructure. Each additional unit adds 30.1 T/s for a marginal \$5,200 and a standard wall outlet.
...
\subsection{Concurrent Session Multiplexing: The Hybrid Attention Dividend}

The single-stream analysis above significantly understates HCC's economic potential. GLM-5.3-Flash's hybrid attention compresses the per-node KV cache to just $\sim$1.27 GB per session at 200K context (or $\sim$5.18 GB under full MLA). With $\sim$86 GB of free memory after weights and OS across the dual-node cluster, this enables \textbf{up to 33 concurrent sessions} (86 $\div$ 2.54 $\approx$ 33.8). At shorter contexts (32K), the KV cache drops to $\sim$0.4 GB per session, supporting $>100$ concurrent sessions.
...
\begin{table}[ht!]
\centering
\caption{Projected deployment tiers enabled by Hybrid Attention + HCC.\protect\footnotemark}
\vspace{0.2cm}
\begin{tabular}{p{2.3cm} p{3.2cm} r r r p{4.8cm}}
\toprule
\textbf{Tier} & \textbf{Config} & \textbf{Eff.\ T/s} & \textbf{\$/M tok} & \textbf{Cost} & \textbf{Use Case} \\
\midrule
Lite & 1-node, IQ3\_XXS & 21.9$^\dagger$ & \$2.97 & \$2,600 & Async, summarization \\
Standard & 2-node + HCC & 30.1$^\dagger$ & \$2.16 & \$5,200 & Interactive ($<$200ms) \\
Batch-2 & 2-node + \texttt{--parallel 2} & 43.7$^\dagger$ & \$1.49 & \$5,200 & 2 concurrent users \\
\textbf{Multi-session} & \textbf{2-node + batch=4} & \textbf{120}$^\dagger$ & \textbf{\$0.54} & \$5,200 & API serving; e.g., sovereign CSR \cite{csr} compliance pipelines ($<$\$0.01/scan) \\
\bottomrule
\multicolumn{6}{p{\linewidth}}{\footnotesize $^\dagger$Theoretical projection. Batch-2 uses \texttt{llama.cpp --parallel 2}. Multi-session assumes batch=4 with HCC speculative amortization. Batch=4 allocates $4 \times 2.54\text{ GB} = 10.2\text{ GB}$ KV cache across the cluster, leaving $\sim$76 GB free for additional sessions.}
\end{tabular}
\label{tab:tiers}
\end{table}
...
\noindent At the multi-session tier, HCC projects \textbf{\$0.54/M tokens}---\textbf{4.9$\times$ cheaper} than the DGX A100 at full vLLM utilization (\$2.66/M) and \textbf{5.7$\times$ cheaper} than the DGX H100 (\$3.07/M), on \$5,200 of hardware drawing 240W from a wall outlet. This is the complete inversion of the data center cost advantage.

\textbf{Projected NPU+GPU+Dual-USB4 tiers.} Under the stated assumptions, the dual APU cluster projects \$2.97/M tokens without speculative amortization and \$2.16/M (single-stream) to \$0.54/M (multi-session) with it. These figures are sensitivity-model outputs, not evidence that the platform is the most cost-effective option for a deployed workload.
...
\begin{itemize}
    \item \textbf{H1 (Prefill Latency Mitigation):} Context compression cascade ($\alpha = 0.25$) will reduce total TTFT over USB4 by at least 60\%.
    \item \textbf{H2 (Speculative Network Amortization):} Speculative verification with bounded $\gamma=2$ ($v_2 \approx 1.49$) will increase effective decode throughput by at least $1.35\times$ (from $\sim$21.9 to $\ge$29.5 T/s).
    \item \textbf{H3 (KV Fidelity):} 3-bit TurboQuant compression across the 11 DSA layers will maintain retrieval accuracy within 5\% of the FP16 baseline up to 200K tokens.
\end{itemize}
...
\midrule
\textbf{H1} & Measure TTFT at context lengths 1K, 10K, 50K, 100K with and without context compression ($\alpha = 0.25$). Minimum 100 runs per config. & TTFT P50/P95 reduction $\geq$60\% for contexts $>$50K tokens. & Na\"ive \texttt{llama.cpp} RPC (no compression, full payload transfer). \\
\midrule
\textbf{H2} & Measure sustained T/s over 10-minute windows with speculative verification ($\gamma = 2, v_2 \approx 1.49$) enabled vs.\ disabled. & Effective T/s $\geq 1.35\times$ baseline ($>$29.5 T/s target). & Single-node decode rate ($\sim$21.9 T/s roofline bound). \\
\midrule
\textbf{H3} & Run Needle-in-a-Haystack at 32K/64K/128K/200K and MMLU 5-shot with 3-bit TurboQuant KV vs.\ FP16 KV on 11 DSA layers. & Retrieval accuracy within 5\% of FP16; MMLU $\Delta < 2$ points. & GLM-5.3-Flash with full FP16 KV cache (reference). \\
\bottomrule
\end{tabular}
...
\textbf{Theoretical, not empirical.} All performance projections (1.38$\times$ decode speedup to 30.1 T/s, 92\% TCO reduction, \$2.16/M tokens) are derived from roofline models and analytical bounds. Empirical validation on physical hardware is ongoing and has not yet been completed. The experimental protocol in Section~\ref{sec:validation} defines the path to empirical confirmation.
...
\textbf{Draft model alignment.} The projected $\alpha \approx 0.7$ acceptance rate assumes an architecturally aligned draft model or GLM-5.3-Flash's native Multi-Token Prediction (MTP) head. Mismatched draft models may yield lower acceptance rates, reducing the speculative speedup.

\textbf{Model availability and formats.} GLM-5.3-Flash is an open-weight 320B foundation model. While base checkpoints are available in standard precisions, the specialized ROCmFP4 format (\texttt{Q4\_0\_ROCMFP4\_STRIX\_LEAN}) requires export through compatible quantization toolchains.
...
The \textit{Heterogeneous Compute Cascade} (HCC) treats the dual-node setup not as a uniform compute pool, but as a specialized pipeline. The analytical framework combines hybrid Kangaroo Linear Attention and DeepSeek Sparse Attention (KDA+DSA), speculative network amortization with bounded $\gamma=2$, planned zero-copy buffers, and 3-bit KV compression to host a 320B MoE model (GLM-5.3-Flash) on decentralized consumer hardware. The current repository validates the typed two-process protocol, exact residual rejection sampling, and single-node benchmark paths. The projected TCO and throughput advantages remain hypotheses for the experimental roadmap in Section~\ref{sec:validation}.
...
\bibitem{glm5} Zhipu AI, ``GLM-5.3-Flash: Frontier Open-Weight Hybrid MoE with Kangaroo Linear Attention and DeepSeek Sparse Attention,'' \textit{arXiv preprint arXiv:2602.15763 / Tech Report}, 2026.
...
The following derivation shows how the deployed GLM-5.3-Flash checkpoint size is obtained from public specifications and standard quantization bit-widths:

\begin{table}[ht!]
\centering
\caption{GLM-5.3-Flash checkpoint size reconciliation.}
\vspace{0.2cm}
\begin{tabular}{p{5.5cm} r l}
\toprule
\textbf{Step} & \textbf{Size} & \textbf{Specification / Source} \\
\midrule
GLM-5.3-Flash BF16 (full model) & 642 GB & 320B total parameters $\times$ 2.01 B/w \\
GLM-5.3-Flash FP8 (full model) & 321 GB & 320B total parameters $\times$ 1.00 B/w \\
\midrule
\multicolumn{3}{l}{\textit{Architecture and Sparsity:}} \\
Total parameters & $\sim$320B & 45 layers (34 KDA + 11 DSA) \\
Routed experts & 288 & Top-8 activated per token ($K=8$) \\
Active parameters per token & $\sim$18B & 18.0B forward pass footprint \\
\midrule
\multicolumn{3}{l}{\textit{Quantization Formats:}} \\
Q4\_0\_ROCMFP4\_STRIX\_LEAN & \textbf{158.0 GB} & $\sim$4.3 bpw ($\sim$79.0 GB per node) \\
UD-IQ3\_XXS (single-node) & \textbf{120.0 GB} & Fits single 128 GB node \\
STRIX-BALANCED (compact) & \textbf{72.0 GB} & Fits compact 96 GB allocation \\
\midrule
\multicolumn{3}{l}{\textit{Dual-Node Deployment Fit Check:}} \\
Cluster memory (2$\times$128 GB) & 256.0 GB & Hardware specification \\
Model checkpoint (ROCmFP4) & 158.0 GB & Partitioned evenly (79 GB/node) \\
Hybrid DSA KV cache (200K tokens) & 2.5 GB & 1.27 GB per node (11 DSA layers) \\
Full MLA KV cache (200K tokens) & 10.4 GB & 5.18 GB per node (45 layers) \\
Remaining headroom (OS + buffers) & \textbf{87.6--95.4 GB} & Ample headroom for 30+ sessions \\
\bottomrule
\end{tabular}
\label{tab:size_reconciliation}
\end{table}

\noindent \textbf{Key finding:} Unlike dense 744B-class architectures that require destructive expert pruning to fit 256 GB, GLM-5.3-Flash fits natively under ROCmFP4 (\texttt{Q4\_0\_ROCMFP4\_STRIX\_LEAN}) at 158.0 GB gross without pruning any of its 288 experts, preserving 100\% of model reasoning capacity while retaining over 87 GB of memory headroom across the dual-node cluster.
...
\textbf{Forward-Looking Statements.} This paper contains forward-looking statements regarding projected performance, cost efficiency, throughput, and deployment feasibility of the Heterogeneous Compute Cascade (HCC) architecture. These projections are derived from roofline models, theoretical bounds, and published hardware specifications under idealized conditions. Actual results may differ materially due to, but not limited to: software implementation complexity, hardware manufacturing variability, thermal throttling under sustained loads, evolving vendor software support (ROCm, Vitis AI, ONNX Runtime), changes in third-party model availability (GLM-5.3-Flash, ROCmFP4, quantization toolchains), market pricing fluctuations, and regulatory or supply chain constraints. No representation is made that the projected performance will be achieved in any specific deployment.
