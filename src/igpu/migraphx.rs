...
fn lib() -> &'static Library {
    MIGRAPHX_LIB
        .get_or_init(|| unsafe { Library::new("/opt/rocm/lib/libmigraphx_c.so.3").unwrap() })
}

type MigraphxCreateFn = unsafe extern "C" fn() -> *mut std::ffi::c_void;
type MigraphxDestroyFn = unsafe extern "C" fn(*mut std::ffi::c_void);
type MigraphxRunFn =
    unsafe extern "C" fn(*mut std::ffi::c_void, *const f32, *mut f32, usize, usize) -> i32;
...
pub fn hipblaslt_matmul(
    ...
    let run: Symbol<
        unsafe extern "C" fn(*const f32, *const f32, *mut f32, usize, usize, usize) -> i32,
